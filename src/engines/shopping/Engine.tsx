...
export function ShoppingEngine({ params, api, onProgress, onComplete }: EngineProps<ShoppingParams>) {
    ...
    timer.current = setTimeout(() => {
      if (round + 1 === params.orders.length) {
        ...
        onComplete({ completed: true, durationMs: Date.now() - started.current,
          correct: params.orders.length, attempts: attempts.current, hintsUsed: 0 });
      } else {
        setRound(round + 1); setBasket([]); setReady(false); setMessage(''); locked.current = false;
      }
    }, 1400);
  }
  return <div className={s.shop}>
    ...
    <h2>{order.title}</h2>
    <p>Погледни списъка и напълни кошницата с точните количества.</p>
    <div className={s.list} aria-label="Списък за пазаруване">
    ...
    <div className={s.shelf} role="group" aria-label="Продукти на пазара">
      {params.shelf.map(asset => <button key={asset} type="button" aria-label={`Добави ${assetLabel(asset)}`}
        disabled={ready || basket.length >= 18} onClick={() => { setBasket(items => [...items, asset]); api.sfx('pick'); }}>
    ...
    <button className={s.checkout} type="button" disabled={ready || !basket.length} onClick={checkout}>
